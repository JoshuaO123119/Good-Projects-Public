...
public class ticTacToeAI4x4
{
		...
		while (true) {
			...
			while (gameRunning) {
				...
				if (turn.equals("X")) {
					...
					System.out.println("Type a column (a/b/c/d): ");
					...
					System.out.println("Type a row (1/2/3/4): ");
					int c = input.nextInt();
					if (c < 5 && c > 0) {
	...
	private static String validateWin(String[][] board) {
		...
		for (String player : players) {
			for (int k = 0; k < 2; k++) {
				for (int i = 0; i < board.length; i++) {
					if (board[i][0+k].equals(player) && board[i][1+k].equals(player) && board[i][2+k].equals(player)) {
						return player;
					}
				}
			}
		...
		for (String player : players) {
			for (int k = 0; k < 2; k++) {
				for (int i = 0; i < board.length; i++) {
					for (int j = 0; j < board[i].length; j++) {
						if (board[0+k][j].equals(player) && board[1+k][j].equals(player) && board[2+k][j].equals(player)) {
						return player;
						}
					}
				}
		...
		// bottom left to up right pattern
		for (String player: players) {
			for (int i = 0; i < board.length; i++) {
				for (int j = 0; j < board[i].length; j++) {
					try { // Check for pattern
						if (board[i][j].equals(player) && board[i+1][j-1].equals(player) && board[i+2][j-2].equals(player)) {
							// If found player won
							return player;
						}
					} catch (Exception e) {
		...
		// bottom right to up left pattern
		for (String player: players) {
			for (int i = 0; i < board.length; i++) {
				for (int j = 0; j < board[i].length; j++) {
					try { // Check for pattern
